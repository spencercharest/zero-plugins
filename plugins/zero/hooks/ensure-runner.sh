...
# Isolated $HOME for the runner: a fresh, plugin-owned config dir ($RUNNER_HOME/.zero)
# so the runner NEVER reads the user's ~/.zero wallet/config. Lives under the plugin
# data dir, so it's per-install and removed on uninstall. Auth comes from the MCP-minted
# ZERO_SESSION_TOKEN (managed wallet); an explicit ZERO_PRIVATE_KEY is still honored (BYO).
RUNNER_HOME="$DATA_DIR/home"
...
# Runs the bundled JS runner (zero.mjs) on a resolved Node runtime with an isolated
# HOME, so the runner uses a plugin-owned config dir ($RUNNER_HOME/.zero) instead of the
# user's ~/.zero. Auth/signing come from the MCP-minted ZERO_SESSION_TOKEN (the
# connector's managed wallet); an explicit ZERO_PRIVATE_KEY is still honored for BYO.
exec env HOME="$RUNNER_HOME" "$NODE_BIN" "$MJS_PATH" "\$@"
SHIM
    chmod +x "$SHIM_PATH" 2>/dev/null || true
    persist_env ZERO_RUNNER "$SHIM_PATH"
    persist_path "$BIN_DIR"
    emit "Zero runner is ready: ZERO_RUNNER=$SHIM_PATH is a drop-in for the zero CLI (it runs the bundled zero.mjs on Node; the bundle tracks the v$MAJOR channel and auto-updates, re-checked about every ${RUNNER_REFRESH_TTL_MIN} minutes). Run the whole loop through it. Authenticate ONLY by minting a credential with the Zero MCP tool mint_runner_session (returns { token, walletAddress, expiresAt, budgetUsdc }, backed by the user's connector wallet) and pass it as ZERO_SESSION_TOKEN, e.g. ZERO_SESSION_TOKEN=<token> $SHIM_PATH fetch <url>. NEVER run 'zero init', 'zero auth login', 'zero wallet', or 'zero welcome', and never mention a welcome bonus. If mint_runner_session is unavailable or errors that the connector isn't authorized, walk the user through authorizing the Zero MCP connector (OAuth) in their client's connector settings — do not create or use a local wallet. The token is short-lived (~5 min, see expiresAt) with a per-token spend cap (default 5 USDC, see budgetUsdc); re-mint when it expires or the budget is spent. The runner uses an isolated, plugin-owned config dir (not your ~/.zero), so it never picks up a local or CLI-created wallet — the connector's managed wallet signs. An explicit ZERO_PRIVATE_KEY in the environment is still honored for bring-your-own signing, but never create a wallet yourself. mint_runner_session is the only MCP tool you call; search, get, fetch and review all go through the runner."
    exit 0
fi
